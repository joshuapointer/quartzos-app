...
- **Maximalist RGB / cyberpunk.** Glowing neon edges on everything, scanlines, animated gradient backgrounds. The aesthetic is theatrical glass + chromatic dispersion, not gamer setup.
...
5. **One thing at a time.** During the 90 seconds that matter, the screen serves the current moment, pre-torch, heating, in-window, post-dab. Modes are mutually exclusive. The user shouldn't need to read to know which moment they're in. The single-surface architecture (the Altar) enforces this at the structural level: no tabs, no sheets, no modals competing with the orb.

## Accessibility & Inclusion

No specific WCAG target stated. Sensible defaults:

- **Contrast.** Bone-100 `#e8ecf2` on cool obsidian `#060507` clears WCAG AA for body and AAA for headlines; bone-60 `#adb1b7` on the same base passes AA for non-text UI but should be reserved for secondary copy. Telemetry uses bone-100 on glass surfaces (`rgba(252,252,255,0.05)`) for legibility under varying ambient brightness.
- **Motion.** Respect `prefers-reduced-motion` for the orb pulse, prism-drift edges (9s loop), ambient bloom layers, and any phase transition longer than 240ms. The orb is allowed to dim and resize, but not pulse, when reduced motion is requested.
- **Color is never the sole signal for window state.** The orb's hue carries it (cyan idle, dispersing prism in window), but text and motion redundantly encode the same moment, the temp readout, the eyebrow label, the orb's scale.
- **Type.** Support Dynamic Type / system font sizing where it doesn't break the dial geometry. The serif display (Instrument Serif Italic 96) and mono labels (Geist Mono caps) hold legibility down to 80% of native scale; below that, fall back to bone-80 weight bumps rather than further size reduction.
